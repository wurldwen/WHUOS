...
- **相关宏**:
  - SATP_SV39: MODE = SV39
  - MAKE_SATP(pagetable): 设置MODE和PPN字段
  - VA_TO_VPN(va, level): 获取虚拟地址中的虚拟页(VPN)信息
  - PA_TO_PTE(pa): PA和PTE之间的转换
  - PTE_TO_PA(pte): PTE到PA的转换
  - PTE_V, PTE_R, PTE_W, PTE_X, PTE_U, PTE_G, PTE_A, PTE_D: 权限位
...
- **测试用例**:
  - 分别用内核模拟分配和用户态大量分配来验证内核保留页不会被耗尽。
  - 验证 `pmem_free` 在错误 region/未对齐地址时会触发 panic。
- **进一步改进**:
  - 可在 `pmem.c` 中添加统计接口（返回 `allocable`）。
  - 为用户区实现配额/回收策略（避免短时间内用户耗尽全部用户页导致系统进程饿死）。
...
`pmem.c` 实现了物理内存分配器，使用两个 `alloc_region_t` 池（内核 / 用户）。每个空闲的物理页在页首存放一个 `page_node_t`，用于链表连接，每个区域维护一个哨兵链表头。

### 关键函数

- `pmem_init()`: 初始化内核和用户区域，按 `KERNEL_PAGES` 划分内存。
- `pmem_alloc(bool in_kernel)`: 从指定区域分配一页。
- `pmem_free(uint64 page, bool in_kernel)`: 释放一页到指定区域。
...
`vmem.c` 实现了基于 SV39 的简洁页表辅助。

### 关键函数

- `vm_getpte(pgtbl_t pgtbl, uint64 va, bool alloc)`: 遍历页表并返回对应虚拟地址的 PTE 指针，支持分配中间页表。
- `vm_mappages(pgtbl_t pgtbl, uint64 va, uint64 pa, uint64 len, int perm)`: 映射虚拟地址区间到物理地址区间。
- `vm_unmappages(pgtbl_t pgtbl, uint64 va, uint64 len, bool freeit)`: 解除映射，支持释放物理页。
- `kvm_init()`: 创建内核页表并映射设备与内核区域。
- `kvm_inithart()`: 激活页表。

### 实现细节

- 内核页表恒等映射设备 MMIO 和内核代码/数据段。
- 使用 `sfence_vma()` 确保 TLB 一致性。
