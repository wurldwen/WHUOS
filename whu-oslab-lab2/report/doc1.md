# WHU OS Lab 2: 虚拟内存管理实验报告

## 概述

本实验报告详细记录了基于 RISC-V 架构的操作系统内核中物理内存分配器和虚拟内存管理模块的实现过程。通过分析 xv6 操作系统的内存管理机制，我们重新设计并实现了改进的内存管理模块，包括物理内存分配器（pmem.c）和虚拟内存管理器（vmem.c）。报告将从关键数据结构开始，逐步深入到源码分析、实现过程、问题解决以及测试验证。
...
- **相关宏**:
  ...
  - **权限位**: PTE_V, PTE_R, PTE_W, PTE_X, PTE_U, PTE_G, PTE_A, PTE_D
...
#### 5. 参考 xv6 的内核初始化：`kvminit()` 和 `kvminithart()`
